...
public class PricesApi {
 ...
 /**
   ...
   * @param ids The requested list of security identifiers. Accepted ID types include Market Tickers, SEDOL, ISINs, CUSIPs, or FactSet Permanent IDs. **IDs limit** &#x3D; 500 per non-batch request / 2000 per batch request for a single day and 50 per multi-day request for both batch and non-batch. The number of unique currencies present in the requested IDs is limited to 50 per request. Make note, GET Method URL request lines are also limited to a total length of 8192 bytes (8KB). In cases where the service allows for thousands of IDs, which may lead to exceeding this request line limit of 8KB, it&#39;s advised for any requests with large request lines to be requested through the respective \&quot;POST\&quot; method. (required)
   ...
   * @param adjust Controls the split and spinoff adjustments for the prices.   * **SPLIT** &#x3D; Split ONLY Adjusted. This is used by default.   * **SPLIT_SPINOFF** &#x3D; Splits and Spinoff Adjusted.   * **DIV_SPIN_SPLITS** &#x3D; Dividend adjustments, Spinoff, and Splits combined.   * **UNSPLIT** &#x3D; No Adjustments.     (optional, default to SPLIT)
   * @param precision Specifies the number of decimal places for price fields in the response.  (optional, default to 16)
   ...
   */
  public GetGPDPricesResponseWrapper getGPDPrices(java.util.List<String> ids, String startDate, java.util.List<String> fields, String endDate, String frequency, String calendar, String currency, String adjust, Integer precision, String batch) throws ApiException {
    return getGPDPricesWithHttpInfo(ids, startDate, fields, endDate, frequency, calendar, currency, adjust, precision, batch).getData();
  }
   ...
   */
  public ApiResponse<GetGPDPricesResponseWrapper> getGPDPricesWithHttpInfo(java.util.List<String> ids, String startDate, java.util.List<String> fields, String endDate, String frequency, String calendar, String currency, String adjust, Integer precision, String batch) throws ApiException {
    ...
    localVarQueryParams.addAll(apiClient.parameterToPairs("", "precision", precision));
