"""
    Analytics Datastore  API

    Allow clients to fetch precalculated Analytics through predeterministic URLs.  # noqa: E501

    The version of the OpenAPI document: 1
    Contact: analytics.api.support@factset.com
    Generated by: https://openapi-generator.tech
"""


import re  # noqa: F401
import sys  # noqa: F401
from multiprocessing.pool import ApplyResult
import typing

from fds.sdk.AnalyticsDatastore.api_client import ApiClient, Endpoint as _Endpoint
from fds.sdk.AnalyticsDatastore.model_utils import (  # noqa: F401
    check_allowed_values,
    check_validations,
    date,
    datetime,
    file_type,
    none_type,
    validate_and_convert_types
)
from fds.sdk.AnalyticsDatastore.exceptions import ApiException





class SwivelApi(object):
    """NOTE: This class is auto generated by OpenAPI Generator
    Ref: https://openapi-generator.tech

    Do not edit the class manually.
    """

    def __init__(self, api_client=None):
        if api_client is None:
            api_client = ApiClient()
        self.api_client = api_client
        self.get_swivel_endpoint_endpoint = _Endpoint(
            settings={
                'response_type': (
                  { 200: ([str],),  },
                  None
                ),
                'auth': [
                    'FactSetApiKey',
                    'FactSetOAuth2'
                ],
                'endpoint_path': '/analytics/pub-datastore/swivel/v1/{pubDoc}/{assetName}/{reportId}/{tileId}/{accountId}',
                'operation_id': 'get_swivel_endpoint',
                'http_method': 'GET',
                'servers': None,
            },
            params_map={
                'all': [
                    'pub_doc',
                    'asset_name',
                    'report_id',
                    'tile_id',
                    'account_id',
                ],
                'required': [
                    'pub_doc',
                    'asset_name',
                    'report_id',
                    'tile_id',
                    'account_id',
                ],
                'nullable': [
                ],
                'enum': [
                ],
                'validation': [
                ]
            },
            root_map={
                'validations': {
                },
                'allowed_values': {
                },
                'openapi_types': {
                    'pub_doc':
                        (str,),
                    'asset_name':
                        (str,),
                    'report_id':
                        (str,),
                    'tile_id':
                        (str,),
                    'account_id':
                        (str,),
                },
                'attribute_map': {
                    'pub_doc': 'pubDoc',
                    'asset_name': 'assetName',
                    'report_id': 'reportId',
                    'tile_id': 'tileId',
                    'account_id': 'accountId',
                },
                'location_map': {
                    'pub_doc': 'path',
                    'asset_name': 'path',
                    'report_id': 'path',
                    'tile_id': 'path',
                    'account_id': 'path',
                },
                'collection_format_map': {
                }
            },
            headers_map={
                'accept': [
                    'headers'
                ],
                'content_type': [],
            },
            api_client=api_client
        )

    @staticmethod
    def apply_kwargs_defaults(kwargs, return_http_data_only, async_req):
        kwargs["async_req"] = async_req
        kwargs["_return_http_data_only"] = return_http_data_only
        kwargs["_preload_content"] = kwargs.get("_preload_content", True)
        kwargs["_request_timeout"] = kwargs.get("_request_timeout", None)
        kwargs["_check_input_type"] = kwargs.get("_check_input_type", True)
        kwargs["_check_return_type"] = kwargs.get("_check_return_type", True)
        kwargs["_spec_property_naming"] = kwargs.get("_spec_property_naming", False)
        kwargs["_content_type"] = kwargs.get("_content_type")
        kwargs["_host_index"] = kwargs.get("_host_index")

    def get_swivel_endpoint(
        self,
        pub_doc="Analytics_Datastore",
        asset_name="ADS_Demo.PA3",
        report_id="report7",
        tile_id="tile0",
        account_id="LARGE_CORE",
        **kwargs
    ) -> [str]:
        """Get Swivel endpoint, takes report parameter inputs and returns the Cargo location for the specified report via a 302 redirect.  # noqa: E501

        Note: Due to Swagger UI functionality, the redirect is automatically followed when using 'Try it out', so instead of the 302 Header response, a 200 is returned with the Cargo response Body.  # noqa: E501
        This method makes a synchronous HTTP request. Returns the http data only

        Args:
            pub_doc (str): The publisher document containing the report. defaults to "Analytics_Datastore", must be one of ["Analytics_Datastore"]
            asset_name (str): The PA/SPAR asset name and extension. defaults to "ADS_Demo.PA3", must be one of ["ADS_Demo.PA3"]
            report_id (str): The PA/SPAR report ID. defaults to "report7", must be one of ["report7"]
            tile_id (str): The PA/SPAR tile ID. defaults to "tile0", must be one of ["tile0"]
            account_id (str): The account ID. defaults to "LARGE_CORE", must be one of ["LARGE_CORE"]

        Keyword Args:
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
        Returns:
            [str]
                Response Object
        """
        self.apply_kwargs_defaults(kwargs=kwargs, return_http_data_only=True, async_req=False)
        kwargs['pub_doc'] = \
            pub_doc
        kwargs['asset_name'] = \
            asset_name
        kwargs['report_id'] = \
            report_id
        kwargs['tile_id'] = \
            tile_id
        kwargs['account_id'] = \
            account_id
        return self.get_swivel_endpoint_endpoint.call_with_http_info(**kwargs)

    def get_swivel_endpoint_with_http_info(
        self,
        pub_doc="Analytics_Datastore",
        asset_name="ADS_Demo.PA3",
        report_id="report7",
        tile_id="tile0",
        account_id="LARGE_CORE",
        **kwargs
    ) -> typing.Tuple[[str], int, typing.MutableMapping]:
        """Get Swivel endpoint, takes report parameter inputs and returns the Cargo location for the specified report via a 302 redirect.  # noqa: E501

        Note: Due to Swagger UI functionality, the redirect is automatically followed when using 'Try it out', so instead of the 302 Header response, a 200 is returned with the Cargo response Body.  # noqa: E501
        This method makes a synchronous HTTP request. Returns http data, http status and headers

        Args:
            pub_doc (str): The publisher document containing the report. defaults to "Analytics_Datastore", must be one of ["Analytics_Datastore"]
            asset_name (str): The PA/SPAR asset name and extension. defaults to "ADS_Demo.PA3", must be one of ["ADS_Demo.PA3"]
            report_id (str): The PA/SPAR report ID. defaults to "report7", must be one of ["report7"]
            tile_id (str): The PA/SPAR tile ID. defaults to "tile0", must be one of ["tile0"]
            account_id (str): The account ID. defaults to "LARGE_CORE", must be one of ["LARGE_CORE"]

        Keyword Args:
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
        Returns:
            [str]
                Response Object
            int
                Http Status Code
            dict
                Dictionary of the response headers
        """
        self.apply_kwargs_defaults(kwargs=kwargs, return_http_data_only=False, async_req=False)
        kwargs['pub_doc'] = \
            pub_doc
        kwargs['asset_name'] = \
            asset_name
        kwargs['report_id'] = \
            report_id
        kwargs['tile_id'] = \
            tile_id
        kwargs['account_id'] = \
            account_id
        return self.get_swivel_endpoint_endpoint.call_with_http_info(**kwargs)

    def get_swivel_endpoint_async(
        self,
        pub_doc="Analytics_Datastore",
        asset_name="ADS_Demo.PA3",
        report_id="report7",
        tile_id="tile0",
        account_id="LARGE_CORE",
        **kwargs
    ) -> "ApplyResult[[str]]":
        """Get Swivel endpoint, takes report parameter inputs and returns the Cargo location for the specified report via a 302 redirect.  # noqa: E501

        Note: Due to Swagger UI functionality, the redirect is automatically followed when using 'Try it out', so instead of the 302 Header response, a 200 is returned with the Cargo response Body.  # noqa: E501
        This method makes a asynchronous HTTP request. Returns the http data, wrapped in ApplyResult

        Args:
            pub_doc (str): The publisher document containing the report. defaults to "Analytics_Datastore", must be one of ["Analytics_Datastore"]
            asset_name (str): The PA/SPAR asset name and extension. defaults to "ADS_Demo.PA3", must be one of ["ADS_Demo.PA3"]
            report_id (str): The PA/SPAR report ID. defaults to "report7", must be one of ["report7"]
            tile_id (str): The PA/SPAR tile ID. defaults to "tile0", must be one of ["tile0"]
            account_id (str): The account ID. defaults to "LARGE_CORE", must be one of ["LARGE_CORE"]

        Keyword Args:
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
        Returns:
            ApplyResult[[str]]
        """
        self.apply_kwargs_defaults(kwargs=kwargs, return_http_data_only=True, async_req=True)
        kwargs['pub_doc'] = \
            pub_doc
        kwargs['asset_name'] = \
            asset_name
        kwargs['report_id'] = \
            report_id
        kwargs['tile_id'] = \
            tile_id
        kwargs['account_id'] = \
            account_id
        return self.get_swivel_endpoint_endpoint.call_with_http_info(**kwargs)

    def get_swivel_endpoint_with_http_info_async(
        self,
        pub_doc="Analytics_Datastore",
        asset_name="ADS_Demo.PA3",
        report_id="report7",
        tile_id="tile0",
        account_id="LARGE_CORE",
        **kwargs
    ) -> "ApplyResult[typing.Tuple[[str], int, typing.MutableMapping]]":
        """Get Swivel endpoint, takes report parameter inputs and returns the Cargo location for the specified report via a 302 redirect.  # noqa: E501

        Note: Due to Swagger UI functionality, the redirect is automatically followed when using 'Try it out', so instead of the 302 Header response, a 200 is returned with the Cargo response Body.  # noqa: E501
        This method makes a asynchronous HTTP request. Returns http data, http status and headers, wrapped in ApplyResult

        Args:
            pub_doc (str): The publisher document containing the report. defaults to "Analytics_Datastore", must be one of ["Analytics_Datastore"]
            asset_name (str): The PA/SPAR asset name and extension. defaults to "ADS_Demo.PA3", must be one of ["ADS_Demo.PA3"]
            report_id (str): The PA/SPAR report ID. defaults to "report7", must be one of ["report7"]
            tile_id (str): The PA/SPAR tile ID. defaults to "tile0", must be one of ["tile0"]
            account_id (str): The account ID. defaults to "LARGE_CORE", must be one of ["LARGE_CORE"]

        Keyword Args:
            _preload_content (bool): if False, the urllib3.HTTPResponse object
                will be returned without reading/decoding response data.
                Default is True.
            _request_timeout (int/float/tuple): timeout setting for this request. If
                one number provided, it will be total request timeout. It can also
                be a pair (tuple) of (connection, read) timeouts.
                Default is None.
            _check_input_type (bool): specifies if type checking
                should be done one the data sent to the server.
                Default is True.
            _check_return_type (bool): specifies if type checking
                should be done one the data received from the server.
                Default is True.
            _spec_property_naming (bool): True if the variable names in the input data
                are serialized names, as specified in the OpenAPI document.
                False if the variable names in the input data
                are pythonic names, e.g. snake case (default)
            _content_type (str/None): force body content-type.
                Default is None and content-type will be predicted by allowed
                content-types and body.
            _host_index (int/None): specifies the index of the server
                that we want to use.
                Default is read from the configuration.
        Returns:
            ApplyResult[([str], int, typing.Dict)]
        """
        self.apply_kwargs_defaults(kwargs=kwargs, return_http_data_only=False, async_req=True)
        kwargs['pub_doc'] = \
            pub_doc
        kwargs['asset_name'] = \
            asset_name
        kwargs['report_id'] = \
            report_id
        kwargs['tile_id'] = \
            tile_id
        kwargs['account_id'] = \
            account_id
        return self.get_swivel_endpoint_endpoint.call_with_http_info(**kwargs)

