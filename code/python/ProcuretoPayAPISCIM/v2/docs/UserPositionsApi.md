...
with fds.sdk.ProcuretoPayAPISCIM.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        api_response = api_instance.get_user_position(id)
...
with fds.sdk.ProcuretoPayAPISCIM.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        # example passing only required values which don't have defaults set
        # and optional values
        api_response = api_instance.get_user_positions(filter=filter, sort_by=sort_by, sort_order=sort_order, start_index=start_index, count=count, attributes=attributes, excluded_attributes=excluded_attributes)
