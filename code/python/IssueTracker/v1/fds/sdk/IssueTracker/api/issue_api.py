...
class IssueApi(object):
    ...
    def patch_issue(
        ...
        **kwargs
        ...
        """Update severity and subject of issue or productId and categoryId of issue  # noqa: E501

        User can update either `severity` with `subject` or `productId` with `categoryId`.    **Note:** Users are not allowed to update `severity` with `productId` or `subject` with `productId`  # noqa: E501
        This method makes a synchronous HTTP request. Returns the http data only
