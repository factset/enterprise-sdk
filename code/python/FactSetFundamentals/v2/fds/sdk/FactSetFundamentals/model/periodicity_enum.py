...
class PeriodicityEnum(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Args:
            args[0] (str): Specifies the frequency of fiscal periods for the data requested.  * `ANN`: Annual Original * `ANN_R`: Annual Restated * `QTR`: Quarterly Original * `QTR_R`: Quarterly Restated * `SEMI`: Semi-Annual Original * `SEMI_R`: Semi-Annual Restated * `LTM`: Last Twelve Months (Calculated) * `LTM_R`: Last Twelve Months Restated (Calculated) * `LTM_SEMI`: LTM based on Semi-Annual * `LTM_SEMI_R`: LTM Restated based on Semi-Annual * `LTMSG`: LTM Global (QTR preferred over SEMI) * `YTD`: Year-to-Date . if omitted defaults to "ANN", must be one of ["ANN", "ANN_R", "QTR", "QTR_R", "SEMI", "SEMI_R", "LTM", "LTM_R", "LTM_SEMI", "LTM_SEMI_R", "LTMSG", "YTD", ]  # noqa: E501
