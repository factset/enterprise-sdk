...
[**get_prb_jobs**](JobsApi.md#get_prb_jobs) | **GET** /jobs | Get a list of existing jobs
...
with fds.sdk.PortfolioReportingBatcher.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        # example passing only required values which don't have defaults set
        # and optional values
        api_response = api_instance.get_prb_jobs(type=type, name=name, pagination_offset=pagination_offset, pagination_limit=pagination_limit)

        pprint(api_response)

    except fds.sdk.PortfolioReportingBatcher.ApiException as e:
        print("Exception when calling JobsApi->get_prb_jobs: %s\n" % e)
```
