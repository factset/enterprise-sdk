...
[![API Version](https://img.shields.io/badge/api-v2.2.1-blue)](https://developer.factset.com/api-catalog/global-filings-api)
[![PyPi](https://img.shields.io/pypi/v/fds.sdk.GlobalFilings/2.3.1)](https://pypi.org/project/fds.sdk.GlobalFilings/v/2.3.1)
...
```shell
poetry add fds.sdk.utils fds.sdk.GlobalFilings==2.3.1
```

### pip

```shell
pip install fds.sdk.utils fds.sdk.GlobalFilings==2.3.1
```
...
with fds.sdk.GlobalFilings.ApiClient(configuration) as api_client:
    ...
    # try:
    #     # Returns the count of filings for specified source.
    #     api_response, http_status_code, response_headers = api_instance.get_count_with_http_info(sources, start_date=start_date, end_date=end_date, ids=ids, time_zone=time_zone, categories=categories, primary_id=primary_id, search_text=search_text, form_types=form_types)
    ...
    # try:
    #     # Returns the count of filings for specified source.
    #     async_result = api_instance.get_count_async(sources, start_date=start_date, end_date=end_date, ids=ids, time_zone=time_zone, categories=categories, primary_id=primary_id, search_text=search_text, form_types=form_types)
    ...
    # try:
    #     # Returns the count of filings for specified source.
    #     async_result = api_instance.get_count_with_http_info_async(sources, start_date=start_date, end_date=end_date, ids=ids, time_zone=time_zone, categories=categories, primary_id=primary_id, search_text=search_text, form_types=form_types)
