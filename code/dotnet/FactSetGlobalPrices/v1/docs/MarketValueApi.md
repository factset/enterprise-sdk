# FactSet.SDK.FactSetGlobalPrices.Api.MarketValueApi
...
<a name="getgpdmarketval"></a>
# **GetGPDMarketVal**
> MarketValueResponse GetGPDMarketVal (List<string> ids, string batch = null)
...
namespace Example
{
    public class GetGPDMarketValExample
    {
        public static async Task Main()
        {
            ...
            var ids = new List<string>(); // List<string> | The requested list of security identifiers. Accepted ID types include Market Tickers, SEDOL, ISINs, CUSIPs, or FactSet Permanent IDs.<p>***IDs limit** =  50 per non-batch request and 400 per batch request*</p> *<p>Make note, GET Method URL request lines are also limited to a total length of 8192 bytes (8KB). In cases where the service allows for thousands of IDs, which may lead to exceeding this request line limit of 8KB, it's advised for any requests with large request lines to be requested through the respective \"POST\" method.</p>*
            ...
            {
                // Provides the current market value for a list of specified securities.
                MarketValueApi.GetGPDMarketValResponseWrapper result = apiInstance.GetGPDMarketVal(ids, batch);
            ...
            {
                Console.WriteLine("Exception when calling MarketValueApi.GetGPDMarketVal: " + e.Message );
...
------------- | ------------- | ------------- | -------------
 **ids** | [**List&lt;string&gt;**](string.md)| The requested list of security identifiers. Accepted ID types include Market Tickers, SEDOL, ISINs, CUSIPs, or FactSet Permanent IDs.&lt;p&gt;***IDs limit** &#x3D;  50 per non-batch request and 400 per batch request*&lt;/p&gt; *&lt;p&gt;Make note, GET Method URL request lines are also limited to a total length of 8192 bytes (8KB). In cases where the service allows for thousands of IDs, which may lead to exceeding this request line limit of 8KB, it&#39;s advised for any requests with large request lines to be requested through the respective \&quot;POST\&quot; method.&lt;/p&gt;* | 
...
<a name="getsecuritypricesforlistmarketval"></a>
# **GetSecurityPricesForListMarketVal**
> MarketValueResponse GetSecurityPricesForListMarketVal (MarketValueRequest marketValueRequest)
...
namespace Example
{
    public class GetSecurityPricesForListMarketValExample
    {
        public static async Task Main()
        {
            var config = new FactSet.SDK.FactSetGlobalPrices.Client.Configuration();
            ...
            var marketValueRequest = new MarketValueRequest(); // MarketValueRequest | Request object for `Market Value`.

            try
            {
                // Requests the current market value for a list of specified securities.
                MarketValueApi.GetSecurityPricesForListMarketValResponseWrapper result = apiInstance.GetSecurityPricesForListMarketVal(marketValueRequest);
            ...
            {
                Console.WriteLine("Exception when calling MarketValueApi.GetSecurityPricesForListMarketVal: " + e.Message );
                Console.WriteLine("Status Code: "+ e.ErrorCode);
                Console.WriteLine(e.StackTrace);
            }
        }
