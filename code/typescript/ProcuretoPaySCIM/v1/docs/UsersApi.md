...
> UserResourceList usersGet(opts)
...
```javascript
const { ApiClient, UsersApi } = require('@factset/sdk-procuretopayscim');
...
// Call api endpoint
apiInstance.usersGet(opts).then(
  data => {
...
// Call api endpoint
apiInstance.usersIdGet(id).then(
  data => {
...
> UserResource usersIdPatch(id, opts)
...
// Call api endpoint
apiInstance.usersIdPatch(id, opts).then(
  data => {
...
const apiInstance = new UsersApi();
const id = "id_example"; // String | ID of resource.
const userResource = new procuretopayscim.UserResource(); // UserResource | User resource.
