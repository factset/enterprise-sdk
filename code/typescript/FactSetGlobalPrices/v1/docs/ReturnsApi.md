...
const opts = {
  ...
  'dividendAdjust': "'EXDATE_C'" // String | Controls the dividend reinvestment for the returns calculation.   * **PRICE** = Price Change - Dividends Excluded.   * **EXDATE** = Simple Return - Dividends Received on exdate but not reinvested.   * **EXDATE_C** = Compound Return - Dividends reinvested on exdate.     
};
