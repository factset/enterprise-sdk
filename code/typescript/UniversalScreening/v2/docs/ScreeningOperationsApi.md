...
// Call api endpoint
apiInstance.getExportResults(id).then(
  data => {
...
> SubmitExportResponse submitExport(opts)
...
const apiInstance = new ScreeningOperationsApi();
const opts = {
  'screenExportParameters': new universalscreening.ScreenExportParameters() // ScreenExportParameters | Data required for an export request
};

// Call api endpoint
apiInstance.submitExport(opts).then(
  data => {
