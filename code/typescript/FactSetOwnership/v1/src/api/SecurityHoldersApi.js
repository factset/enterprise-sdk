...
export default class SecurityHoldersApi {
    ...
    */
    ...
    getSecurityHoldersWithHttpInfo(ids, opts) {
      ...
      let returnType = GetSecurityHoldersResponseWrapperTypeMap;
     ...
     * @param {String} opts.currency Currency code for adjusting prices. Default is Local. For a list of currency ISO codes, visit [Online Assistant Page 1470](https://oa.apps.factset.com/pages/1470). (default to 'LOCAL')
     * @param {module:model/String} opts.batch Enables the ability to asynchronously \"batch\" the request, supporting a long-running request for up to 20 minutes.  When `batch=Y`, the service will respond with an HTTP Status Code of 202.  Once a batch request is submitted, use batch status to see if the job has been completed.  Once completed, retrieve the results of the request via batch-result. When using Batch, ids     limit is increased to  1000 ids per request, though limits on query string via GET method still apply.  It's advised to submit large lists of ids via POST method.  (default to 'N')
     * @return { Promise.< GetSecurityHoldersResponseWrapper > } a Promise, with data of type {@link GetSecurityHoldersResponseWrapper }
     */
    ...
    postSecurityHoldersWithHttpInfo(securityHoldersRequest) {
      ...
      let returnType = PostSecurityHoldersResponseWrapperTypeMap;
     ...
     * Gets security ownership details and activity for the requested security identifiers. The services allows filtering by \"Topn\" holders and by holder \"type\", such as Institutions, Insiders, and Stakeholders. 
     * @param {module:model/SecurityHoldersRequest} securityHoldersRequest Requesting Security Holders for a list of Fund Identifiers.
     * @return { Promise.< PostSecurityHoldersResponseWrapper > } a Promise, with data of type {@link PostSecurityHoldersResponseWrapper }
     */
...
const GetSecurityHoldersResponseWrapperTypeMap = {
  200: SecurityHoldersResponse,
  ...
  _createResponseWrapper(statusCode, response) {
    return new GetSecurityHoldersResponseWrapper(statusCode, response);
  }
};

const PostSecurityHoldersResponseWrapperTypeMap = {
  200: SecurityHoldersResponse,
  202: BatchStatusResponse,
  400: ErrorResponse,
  401: ErrorResponse,
  ...
  _createResponseWrapper(statusCode, response) {
    return new PostSecurityHoldersResponseWrapper(statusCode, response);
  }
};
 ...
 * }</pre>
 *
 * @alias module:GetSecurityHoldersResponseWrapper
 * @class
 */
export class GetSecurityHoldersResponseWrapper {
  ...
  /**
   * @returns { SecurityHoldersResponse }
   */
  getResponse200() {
 ...
 * }</pre>
 *
 * @alias module:PostSecurityHoldersResponseWrapper
 * @class
 */
export class PostSecurityHoldersResponseWrapper {
