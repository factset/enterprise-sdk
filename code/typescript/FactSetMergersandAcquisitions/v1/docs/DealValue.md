...
**completionTransactionValue** | **Number** | Returns the transaction value (in USD Millions) at completion for the specified deal identifier. Transaction Value, but calculated instead using the terms of the transaction at completion (vs. announcement). | [optional] 
**completionEnterpriseValue** | **Number** | Returns the enterprise value (in USD Millions) of the transaction at completion, for the specified deal identifier. This is calculated as the total value of the transaction at completion (base equity price at completion), including interest-bearing debt and preferred stock minus cash from the balance sheet. | [optional] 
