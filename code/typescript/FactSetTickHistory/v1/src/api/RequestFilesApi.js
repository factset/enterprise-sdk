/**
 * TickHistory
 * TickHistory provides dynamic access to historical tick data for a specific security for specific dates or date range.
 *
 * The version of the OpenAPI document: 1.1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */


import ApiClient from "../ApiClient";
import RequestFilesResponse from '../model/RequestFilesResponse';
import RequestFilestatus from '../model/RequestFilestatus';

/**
* RequestFiles service.
* @module api/RequestFilesApi
*/
export default class RequestFilesApi {

    /**
    * Constructs a new RequestFilesApi. 
    * @alias module:api/RequestFilesApi
    * @class
    * @param {module:ApiClient} [apiClient] Optional API client implementation to use,
    * default to {@link module:ApiClient#instance} if unspecified.
    */
    constructor(apiClient) {
        this.apiClient = apiClient || ApiClient.instance;
    }



    /**
     * Returns the requestId and status
     * Try it Out - references a Sandbox environment to simulate live reponses.</p> **In Sandbox** data available from 20210101 to 20210131. Current date data is not available.</p> PROD has data from 20120101 to current date &</p> **available IDs is limited to..**</p> US Equity - IBM-USA, F-USA, AAPL-USA, GOOG-USA</p> ASX - CBA-ASX, BHP-ASX</p> LSE - HSBA-LON, VOD-LON</p> Tokyo - 7203-TKS, 4307-TKS</p> Swiss - NESN-SWX, COTN-SWX</p> Singapore - J36-SES &</p> Hong Kong - 700-HKG</p>
     * @param {Date} startDate The date for (or from which) the data is required.</p> **In Sandbox, data available from 1st to 31st Jan'21 only**.
     * @param {Date} endDate The date to which data is required.</p> **In Sandbox, data available from 1st to 31st Jan'21 only**.
     * @param {Object} opts Optional parameters
     * @param {String} opts.ticker The requested list of security identifiers. <p>**Either ticker(s) + iso_code  (OR) product is allowed**</p> **NOTE**:Only one identifier can be requested per request
     * @param {String} opts.isoCode The requested ISO code. <p>ticker(s) + iso_code</p> <p>**Does not support multiple iso_codes**</p>
     * @param {String} opts.fields Fields for which data is requested. Refer <a href= \"https://developer.factset.com/api-catalog/factset-tick-history-api#apiDocumentation\">Output fields list</a>. for available fields.</p> **In trial default=all fields** (default to '*')
     * @param {String} opts.startTime The start time of the data being requested. The time should be in HH:MM:SS format.
     * @param {String} opts.endTime The end time of the data being requested. The time should be in HH:MM:SS format.
     * @param {module:model/String} opts.single Returns data in a single file when single=true. This parameter only works for smaller date/time requests where the file size is upto 128MB. (default to 'false')
     * @param {String} opts.regionalISOCode Works in conjunction with iso_code parameter.<p>ticker(s) + iso_code + regionalISOCode</p> <p>**Does not support multiple regionalISOCodes**</p>
     * @return {Promise} a {@link https://www.promisejs.org/|Promise}, with an object containing data of type {@link module:model/RequestFilesResponse} and HTTP response
     */
    v1RequestFilesGetWithHttpInfo(startDate, endDate, opts) {
      opts = opts || {};
      let postBody = null;
      // verify the required parameter 'startDate' is set
      if (startDate === undefined || startDate === null) {
        throw new Error("Missing the required parameter 'startDate' when calling v1RequestFilesGet");
      }
      // verify the required parameter 'endDate' is set
      if (endDate === undefined || endDate === null) {
        throw new Error("Missing the required parameter 'endDate' when calling v1RequestFilesGet");
      }

      let pathParams = {
      };
      let queryParams = {
        'ticker': opts['ticker'],
        'iso_code': opts['isoCode'],
        'fields': opts['fields'],
        'start_date': startDate,
        'end_date': endDate,
        'start_time': opts['startTime'],
        'end_time': opts['endTime'],
        'single': opts['single'],
        'regionalISOCode': opts['regionalISOCode']
      };
      let headerParams = {
      };
      let formParams = {
      };

      let authNames = ['FactSetApiKey', 'FactSetOAuth2'];
      let contentTypes = [];
      let accepts = ['application/json'];


      let returnType = RequestFilesResponse;

      return this.apiClient.callApi(
        '/v1/request-files', 'GET',
        pathParams, queryParams, headerParams, formParams, postBody,
        authNames, contentTypes, accepts, returnType, null
      );
    }

    /**
     * Returns the requestId and status
     * Try it Out - references a Sandbox environment to simulate live reponses.</p> **In Sandbox** data available from 20210101 to 20210131. Current date data is not available.</p> PROD has data from 20120101 to current date &</p> **available IDs is limited to..**</p> US Equity - IBM-USA, F-USA, AAPL-USA, GOOG-USA</p> ASX - CBA-ASX, BHP-ASX</p> LSE - HSBA-LON, VOD-LON</p> Tokyo - 7203-TKS, 4307-TKS</p> Swiss - NESN-SWX, COTN-SWX</p> Singapore - J36-SES &</p> Hong Kong - 700-HKG</p>
     * @param {Date} startDate The date for (or from which) the data is required.</p> **In Sandbox, data available from 1st to 31st Jan'21 only**.
     * @param {Date} endDate The date to which data is required.</p> **In Sandbox, data available from 1st to 31st Jan'21 only**.
     * @param {Object} opts Optional parameters
     * @param {String} opts.ticker The requested list of security identifiers. <p>**Either ticker(s) + iso_code  (OR) product is allowed**</p> **NOTE**:Only one identifier can be requested per request
     * @param {String} opts.isoCode The requested ISO code. <p>ticker(s) + iso_code</p> <p>**Does not support multiple iso_codes**</p>
     * @param {String} opts.fields Fields for which data is requested. Refer <a href= \"https://developer.factset.com/api-catalog/factset-tick-history-api#apiDocumentation\">Output fields list</a>. for available fields.</p> **In trial default=all fields** (default to '*')
     * @param {String} opts.startTime The start time of the data being requested. The time should be in HH:MM:SS format.
     * @param {String} opts.endTime The end time of the data being requested. The time should be in HH:MM:SS format.
     * @param {module:model/String} opts.single Returns data in a single file when single=true. This parameter only works for smaller date/time requests where the file size is upto 128MB. (default to 'false')
     * @param {String} opts.regionalISOCode Works in conjunction with iso_code parameter.<p>ticker(s) + iso_code + regionalISOCode</p> <p>**Does not support multiple regionalISOCodes**</p>
     * @return { Promise.< module:model/RequestFilesResponse > } a Promise, with data of type {@link module:model/RequestFilesResponse }
     */
    v1RequestFilesGet(startDate, endDate, opts) {
      return this.v1RequestFilesGetWithHttpInfo(startDate, endDate, opts)
        .then(function(response_and_data) {
          return response_and_data.data;
        });
    }


}





