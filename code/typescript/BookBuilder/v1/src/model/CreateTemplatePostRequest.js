/**
 * BookBuilder
 * The BookBuilder API is a powerful tool that combines detailed reports for companies of interest. Save time by using a single source of knowledge instead of relying on multiple APIs to create a presentation ready PDF, also known as a Public Information Book(PIB). </br></br> Consistently create books with the same content/reports for different companies by defining templates with the set of reports saved for re-use. Or, use templates that are created by FactSet for quick use. </br></br> With our powerful API endpoints, you can get the information on companies of interest quickly where and when you need it. </br></br> 
 *
 * The version of the OpenAPI document: 1.13.0
 * Contact: api@factset.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import TemplateContentSection from './TemplateContentSection';

/**
 * The CreateTemplatePostRequest model module.
 * @module model/CreateTemplatePostRequest
 */
class CreateTemplatePostRequest {
    /**
     * Constructs a new <code>CreateTemplatePostRequest</code>.
     * The request body allows users to input the name of the template, ticker
     * @alias module:model/CreateTemplatePostRequest
     */
    constructor() { 
        
        CreateTemplatePostRequest.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>CreateTemplatePostRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/CreateTemplatePostRequest} obj Optional instance to populate.
     * @return {module:model/CreateTemplatePostRequest} The populated <code>CreateTemplatePostRequest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new CreateTemplatePostRequest();

            if (data.hasOwnProperty('tmpl_name')) {
                obj['tmpl_name'] = ApiClient.convertToType(data['tmpl_name'], 'String');
            }
            if (data.hasOwnProperty('tmpl_type')) {
                obj['tmpl_type'] = ApiClient.convertToType(data['tmpl_type'], 'String');
            }
            if (data.hasOwnProperty('content')) {
                obj['content'] = ApiClient.convertToType(data['content'], [TemplateContentSection]);
            }
        }
        return obj;
    }


}

/**
 * name of the template
 * @member {String} tmpl_name
 */
CreateTemplatePostRequest.prototype['tmpl_name'] = undefined;

/**
 * Type of the template (public,pc,pe,fi)
 * @member {String} tmpl_type
 */
CreateTemplatePostRequest.prototype['tmpl_type'] = undefined;

/**
 * Content sections
 * @member {Array.<module:model/TemplateContentSection>} content
 */
CreateTemplatePostRequest.prototype['content'] = undefined;






export default CreateTemplatePostRequest;

