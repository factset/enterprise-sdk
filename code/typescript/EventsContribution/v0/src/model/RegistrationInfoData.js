/**
 * Events Contribution API
 * The Events Contribution API provides the ability to add specific types of corporate events to FactSet’s own events calendar, for display within FactSet’s applications, as well as for off-platform re-distribution via FactSet’s Events Calendar API.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: api@factset.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';

/**
 * The RegistrationInfoData model module.
 * @module model/RegistrationInfoData
 */
class RegistrationInfoData {
    /**
     * Constructs a new <code>RegistrationInfoData</code>.
     * Information regarding event registrant
     * @alias module:model/RegistrationInfoData
     */
    constructor() { 
        
        RegistrationInfoData.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>RegistrationInfoData</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RegistrationInfoData} obj Optional instance to populate.
     * @return {module:model/RegistrationInfoData} The populated <code>RegistrationInfoData</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new RegistrationInfoData();

            if (data.hasOwnProperty('contactEmailAddress')) {
                obj['contactEmailAddress'] = ApiClient.convertToType(data['contactEmailAddress'], 'String');
            }
            if (data.hasOwnProperty('contactName')) {
                obj['contactName'] = ApiClient.convertToType(data['contactName'], 'String');
            }
            if (data.hasOwnProperty('extraInfo')) {
                obj['extraInfo'] = ApiClient.convertToType(data['extraInfo'], 'String');
            }
        }
        return obj;
    }


}

/**
 * Contact email of event registrant
 * @member {String} contactEmailAddress
 */
RegistrationInfoData.prototype['contactEmailAddress'] = undefined;

/**
 * Contact name of event registrant
 * @member {String} contactName
 */
RegistrationInfoData.prototype['contactName'] = undefined;

/**
 * Extra information can be provided regarding event registration. Input must be in English and will be validated upon indication.
 * @member {String} extraInfo
 */
RegistrationInfoData.prototype['extraInfo'] = undefined;






export default RegistrationInfoData;

