# openoptimizer.BarraPortfolioOptimizerApi
...
> SettingsGlobalVarsResponse getGlobalVariablesBPM(opts)
...
```javascript
const { ApiClient, BarraPortfolioOptimizerApi } = require('@factset/sdk-openoptimizer');
...
const apiInstance = new BarraPortfolioOptimizerApi();
const opts = {
  'cacheControl': "cacheControl_example" // String | Standard HTTP header.  Accepts max-stale.
};

// Call api endpoint
apiInstance.getGlobalVariablesBPM(opts).then(
  data => {
...
> ResultResponse getOptimizationResultByIdBPM(id)
...
// Call api endpoint
apiInstance.getOptimizationResultByIdBPM(id).then(
  data => {
...
> OptimizationStatus getOptimizationStatusByIdBPM(id)
...
// Call api endpoint
apiInstance.getOptimizationStatusByIdBPM(id).then(
  data => {
...
> OptimizationStatus postAndOptimizeBPM(opts)
...
const opts = {
  'cacheControl': "cacheControl_example", // String | Standard HTTP header.  Accepts max-stale.
  'bPMOptimizationParametersRequest': {"data":{"strategy":{"id":"Client:/Optimizer/BPM_EXAMPLE1"},"optimization":{"riskModelDate":"2023-10-09","backtestDate":"2023-10-09","cashFlow":"0"},"outputTypes":{"trades":{"identifierType":"cusip","includeCash":true},"holdings":{"identifierType":"cusip","includeCash":true,"excludeZeroShares":false},"stats":false}}} // BPMOptimizationParametersRequest | Optimization Parameters
};

// Call api endpoint
apiInstance.postAndOptimizeBPM(opts).then(
  data => {
...
> AvailableComponentsResponse postComponentsBPM(opts)
...
// Call api endpoint
apiInstance.postComponentsBPM(opts).then(
  data => {
...
> AvailableStrategiesResponse postStrategiesBPM(opts)
...
// Call api endpoint
apiInstance.postStrategiesBPM(opts).then(
  data => {
